  ...
  Future<bool> startRecording() async {
    try {
      ...
      await _audioRecorder.start(
        ...
        // Note: The record package doesn't provide direct access to audio stream
        // We'll simulate audio data for visualization purposes
      );
      ...
      if (_state == RecordingState.recording || _state == RecordingState.paused) {
        ...
        while (retryCount < maxRetries) {
          try {
            ...
            retryCount++;
            if (retryCount >= maxRetries) {
              // If we can't stop gracefully, try to cancel
              try {
                await _audioRecorder.cancel();
              } catch (cancelError) {
                // If cancellation also fails, log the error but continue
                _setError('Failed to stop recording: $e');
              }
              break;
            }
